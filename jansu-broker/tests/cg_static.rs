...
use jansu_broker::{Result, coordinator::group::administrator::Controller};
use jansu_sans_io::{ErrorCode, join_group_request::JoinGroupRequestProtocol};
...
pub async fn fence_conflicting_static_member<G>(
    cluster_id: impl Into<String>,
    broker_id: i32,
    sc: G,
) -> Result<()>
    ...
    G: Storage + Clone,
{
    ...
    let fenced = common::join_group(
        &mut controller,
        Some(CLIENT_ID),
        group_id.as_str(),
        session_timeout_ms,
        rebalance_timeout_ms,
        "conflicting-static-member",
        Some(group_instance_id.as_str()),
        PROTOCOL_TYPE,
        Some(&protocols[..]),
        reason,
    ...
    assert_eq!(
        ...
        ErrorCode::try_from(fenced.error_code)?
    );
    assert_eq!(Some(PROTOCOL_TYPE.into()), fenced.protocol_type);
    assert_eq!(Some(RANGE.into()), fenced.protocol_name);
    assert!(fenced.leader.is_empty());
    assert_eq!("conflicting-static-member", fenced.member_id);
    assert_eq!(0, fenced.members.unwrap().len());

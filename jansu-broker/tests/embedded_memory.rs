...
fn topic_name() -> String {
    format!("embedded-broker-{}", Uuid::now_v7())
}
...
async fn client(bootstrap: Url) -> Result<Client, Box<dyn StdError>> {
    let pool = ConnectionManager::builder(bootstrap)
        .client_id(Some("jansu-broker-embedded-memory".into()))
        .build()
        .await?;
...
#[tokio::test]
async fn embedded_memory_broker_round_trip_and_shutdown() -> TestResult {
    let (cancellation, handle) = start_broker().await?;
    assert_eq!(Some(handle.local_addr.port()), handle.bootstrap.port());
    ...
    assert_eq!(0, produce_one(&client, &topic, b"hello world").await?);

...
use jansu_storage::{Storage, Topition, TxnAddPartitionsRequest};
    ...
    G: Storage + Clone,
{
    ...
    let collect_records = |batches: Vec<jansu_sans_io::record::deflated::Batch>| {
        batches.into_iter().try_fold(Vec::new(), |mut acc, batch| {
    ...
    G: Storage + Clone,
{
    ...
    // Topic name contains "/" but jansu.virtual is NOT set — the slash is just
    ...
    // All six records are returned — jansu.virtual is not set so the "/" is

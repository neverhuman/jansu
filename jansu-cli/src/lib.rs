...
pub enum Error {
    ...
    Proxy(#[from] jansu_proxy::Error),
    Regex(#[from] regex::Error),
    SansIo(#[from] jansu_sans_io::Error),
    Schema(Box<jansu_schema::Error>),
    Server(Box<jansu_broker::Error>),
    Tls(#[from] rustls::Error),
    TlsPkiPem(#[from] rustls::pki_types::pem::Error),
    Topic(#[from] jansu_topic::Error),
    Url(#[from] url::ParseError),
}

impl From<jansu_cat::Error> for Error {
    fn from(value: jansu_cat::Error) -> Self {
        Self::Cat(Box::new(value))
    }
}

impl From<jansu_client::Error> for Error {
    fn from(value: jansu_client::Error) -> Self {
...
impl From<jansu_broker::Error> for Error {
    fn from(value: jansu_broker::Error) -> Self {

    ...
    State: Clone + Send + Sync + 'static,
{
    ...
    async fn serve(
            ...
            topic_names(&request)
                ...
                .filter_map(|topic_name| {
                    self.resource_config
                        .get(topic_name, "jansu.batch.timeout_ms")
        ...
        let max_records = topic_names(&request)
            ...
            .filter_map(|topic_name| {
                self.resource_config
                    .get(topic_name, "jansu.batch.max_records")
...
mod tests {
    ...
    async fn single_request_in_batch() -> Result<(), Error> {
        ...
        let topic_a = "a";
        configuration.put(topic_a, "jansu.batch", "true")?;
        configuration.put(topic_a, "jansu.batch.timeout_ms", "5000")?;

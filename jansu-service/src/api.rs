...
where
    State: Clone + Send + Sync + 'static,
    E: std::error::Error + From<jansu_sans_io::Error> + Send + Sync + 'static,
{
    type Response = Body;
    type Error = E;
...
///
/// A simple example that routes [`MetadataRequest`][`jansu_sans_io::MetadataRequest`]
/// and [`CreateTopicsRequest`][`jansu_sans_io::CreateTopicsRequest`].
/// [`ApiVersionsRequest`][`jansu_sans_io::ApiVersionsRequest`] is created by the
...
/// ```
/// # use rama::Layer as _;
/// # use jansu_sans_io::{CreateTopicsRequest, CreateTopicsResponse, MetadataRequest, MetadataResponse};
/// # use jansu_service::{Error, FrameRouteService, RequestLayer, ResponseService};
/// # #[tokio::main]
...
where
    State: Clone + Send + Sync + 'static,
    E: std::error::Error + From<jansu_sans_io::Error> + From<Error> + Send + Sync + 'static,
{
    pub fn new(
        routes: Arc<BTreeMap<i16, BoxService<State, Frame, Frame, E>>>,
        advertised_versions: Arc<BTreeMap<i16, ApiVersionRange>>,

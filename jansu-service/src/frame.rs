...
where
    S: Service<State, Q>,
    S::Response: Response,
    S::Error: From<jansu_sans_io::Error>,
...
where
    ...
    S::Error: From<jansu_sans_io::Error> + From<tokio::task::JoinError> + Debug,
{
    type Response = Bytes;
    type Error = S::Error;
    ...
    async fn serve(
        ...
        if !self.is_authenticated(api_key) {
            return Err(Into::into(jansu_sans_io::Error::NotAuthenticated));
        }

...
//! Common service layers used in other Jansu crates.
//!
//! ## Overview
//!
//! This crate provides [Layer][`rama::Layer`] and [Service][`rama::Service`]
//! implementations for operating on [`Frame`], [`Body`], [Request][`jansu_sans_io::Request`]
//! and [Response][`jansu_sans_io::Response`].
//!
...
//! implement [`ApiVersionsRequest`][`jansu_sans_io::ApiVersionsRequest`]
//! with valid protocol ranges:
//!
//! ```
//! # use jansu_service::Error;
//! # #[tokio::main]
//! # async fn main() -> Result<(), Error> {
//! # use rama::{Context, Layer as _, Service as _};
//! # use jansu_sans_io::{ApiKey as _, ApiVersionsRequest, MetadataRequest, MetadataResponse};
//! # use jansu_service::{
...
//! routing into `frame_route` (above) to [`MetadataRequest`][`jansu_sans_io::MetadataRequest`]
//! or [`ApiVersionsRequest`][`jansu_sans_io::ApiVersionsRequest`] depending on the
...
//! [`MetadataRequest`][`jansu_sans_io::MetadataRequest`] to initiate a request
//! on the `service`. The request passes through the protocol stack
//! and routed into our service. The service responds with a
//! [`MetadataResponse`][`jansu_sans_io::MetadataResponse`], so that we can
...
//!
//!   assert_eq!(Some("jansu".into()), response.cluster_id);
//! # Ok(())
//! # }
//! ```
//!
//! The [`FrameRouteService`] automatically implements
//! [`ApiVersionsRequest`][`jansu_sans_io::ApiVersionsRequest`]
//! with valid protocol ranges for all defined services. An
//! [`ApiVersionsResponse`][`jansu_sans_io::ApiVersionsResponse`] contains
//! version information for both [`MetadataRequest`][`jansu_sans_io::MetadataRequest`]
//! and [`ApiVersionsRequest`][`jansu_sans_io::ApiVersionsRequest`]:
//!
//! ```
//! # use jansu_service::Error;
//! # #[tokio::main]
...
///
/// ```rust
/// # use jansu_service::{Error, host_port};
/// # use url::Url;
...
pub(crate) static REQUEST_SIZE: LazyLock<Histogram<u64>> = LazyLock::new(|| {
    METER
        .u64_histogram("jansu_request_size")
        .with_unit("By")
...
pub(crate) static RESPONSE_SIZE: LazyLock<Histogram<u64>> = LazyLock::new(|| {
    METER
        .u64_histogram("jansu_response_size")
        .with_unit("By")
...
pub(crate) static REQUEST_DURATION: LazyLock<Histogram<u64>> = LazyLock::new(|| {
    METER
        .u64_histogram("jansu_request_duration")
        .with_unit("ms")
...
pub(crate) static API_REQUESTS: LazyLock<Counter<u64>> = LazyLock::new(|| {
    METER
        .u64_counter("jansu_api_requests")
...
pub(crate) static BYTES_RECEIVED: LazyLock<Counter<u64>> = LazyLock::new(|| {
    METER
        .u64_counter("jansu_bytes_received")

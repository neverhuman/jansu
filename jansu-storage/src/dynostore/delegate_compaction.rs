        ...
        while let Some(meta) = stream.next().await.transpose().map_err(Error::from)? {
            ...
            if !topics.contains_key(topic_name) {
                ...
                if let Ok(Some(metadata)) =
                    self.topic_metadata(&TopicId::Name(topic_name.into())).await
                    && let Some(configs) = &metadata.topic.configs
                {
                ...
                let _ = topics.insert(topic_name.to_string(), retention_ms);
            }

            if let Some(Some(retention)) = topics.get(topic_name)
                && let Ok(age) = now.duration_since(meta.last_modified.into())
                && age > *retention
            {
                self.object_store
    ...
    pub(super) async fn policy_compact(&self) -> Result<u64> {
        ...
        while let Some(meta) = stream.next().await.transpose().map_err(Error::from)? {
            ...
            if let Ok(res) = self.object_store.get(&meta.location).await
                ...
                && let Ok(mut batch) = self.decode(bytes)
            {
                ...
                if let Ok(inflated_batch) = Batch::try_from(&batch) {
        ...
        for (location, topic_name, partition, batch) in all_records {
            ...
            if !keep && self.object_store.delete(&location).await.is_ok() {
                compacted += 1;
            }
        }

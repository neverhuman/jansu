...
use jansu_sans_io::{
    ConfigResource, ConfigSource, ConfigType, ErrorCode, IsolationLevel, ListOffset,
    NULL_TOPIC_ID, OpType, ScramMechanism,
...
use libsql::ffi::SQLITE_CONSTRAINT_UNIQUE;
...
use metrics::{
    CONNECT_DURATION, DELEGATE_REQUEST_DURATION, SQL_DURATION, SQL_REQUESTS, elapsed_millis,
};

mod connection;

use connection::{ConnectionManager, Pool, is_unique_constraint};
...
impl Delegate {
    #[instrument(skip_all)]
    pub(super) async fn connection(&self) -> Result<managed::Object<ConnectionManager>> {

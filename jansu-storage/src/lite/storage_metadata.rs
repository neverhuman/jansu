...
impl Delegate {
    pub(super) async fn delegate_metadata(&self, topics: Option<&[TopicId]>) -> Result<MetadataResponse> {
    ...
    pub(super) async fn delegate_list_groups(&self, states_filter: Option<&[String]>) -> Result<Vec<ListedGroup>> {
        let start = SystemTime::now();

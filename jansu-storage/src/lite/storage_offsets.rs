...
impl Delegate {
    ...
    pub(super) async fn delegate_committed_offset_topitions(&self, group_id: &str) -> Result<BTreeMap<Topition, i64>> {
        let start = SystemTime::now();
    ...
    pub(super) async fn delegate_leader_epoch_history(&self, topition: &Topition) -> Result<Vec<LeaderEpochRecord>> {
        let start = SystemTime::now();

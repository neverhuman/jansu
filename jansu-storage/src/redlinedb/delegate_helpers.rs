...
impl Delegate {
    ...
    #[instrument(skip_all)]
    pub(super) async fn ensure_topition_for_produce(
        &self,
        topition: &Topition,
        ...
        for partition in 0..=topition.partition() {
            ...
            if connection
                .query_opt("topition_select_id.sql", params)
                .await?
                .is_none()
            {
                _ = connection.execute("topition_insert.sql", params).await?;
                _ = connection.execute("watermark_insert.sql", params).await?;
                _ = connection
                    .execute(

...
/// use jansu_sans_io::{DeleteTopicsRequest, DeleteTopicsResponse,
///     delete_topics_response::DeletableTopicResult, ErrorCode};
/// use jansu_storage::{DeleteTopicsService, Error, StorageContainer};
/// use url::Url;
///
/// # #[tokio::main]
    ...
    G: Storage,
{
    ...
    async fn serve(
        ...
        for topic in req.topics.unwrap_or_else(Vec::new) {
        ...
        for topic in req.topic_names.unwrap_or_else(Vec::new) {
            let error_code = ctx.state().delete_topic(&topic.clone().into()).await?;

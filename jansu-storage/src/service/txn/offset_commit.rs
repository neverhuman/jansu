...
use jansu_sans_io::{ApiKey, TxnOffsetCommitResponse};
use rama::{Context, Service};
use tracing::instrument;

use crate::{Error, Result, Storage};

/// A [`Service`] using [`Storage`] as [`Context`] taking [`jansu_sans_io::TxnOffsetCommitRequest`] returning [`TxnOffsetCommitResponse`].
#[derive(Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct OffsetCommitService;

impl ApiKey for OffsetCommitService {
    const KEY: i16 = jansu_sans_io::TxnOffsetCommitRequest::KEY;
}

impl<G> Service<G, jansu_sans_io::TxnOffsetCommitRequest> for OffsetCommitService
where
    G: Storage,
{

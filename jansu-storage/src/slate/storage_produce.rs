...
use super::types::{
    BatchKey, LeaderEpochKey, LeaderEpochKeyPrefix, Producers, Topics, Transactions,
    TxnProduceOffset, Watermark, WatermarkKey,
};
...
impl Engine {
    pub(super) async fn impl_produce(
        ...
        if current_epoch.is_none_or(|current| batch_leader_epoch > current) {
            ...
            let value = postcard::to_stdvec(&super::types::LeaderEpochValue {
                start_offset: offset,
            })?;
            tx.put(key, value)?;
        }
        ...
        if let Some(ref lake) = self.lake {
            ...
            if !attributes.control {
                let config = self
                    .impl_describe_config(topition.topic(), ConfigResource::Topic, None)
                    .await?;

...
impl Storage for ExactnessStorage {
    ...
    async fn incremental_alter_resource(
        ...
        jansu_sans_io::incremental_alter_configs_response::AlterConfigsResourceResponse,
    > {
    ...
    async fn delete_records(
        &self,
        topics: &[jansu_sans_io::delete_records_request::DeleteRecordsTopic],
    ) -> StorageResult<Vec<jansu_sans_io::delete_records_response::DeleteRecordsTopicResult>> {
    ...
    async fn brokers(
        &self,
    ) -> StorageResult<Vec<jansu_sans_io::describe_cluster_response::DescribeClusterBroker>> {
    ...
    async fn fetch(
        ...
        min_bytes: u32,
        max_bytes: u32,
        isolation: jansu_sans_io::IsolationLevel,
    ...
    async fn list_offsets(
        &self,
        isolation_level: jansu_sans_io::IsolationLevel,
        offsets: &[(Topition, jansu_sans_io::ListOffset)],
    ...
    async fn upsert_user_scram_credential(
        &self,
        user: &str,
        mechanism: jansu_sans_io::ScramMechanism,
    ...
    ) -> StorageResult<Vec<jansu_sans_io::list_groups_response::ListedGroup>> {
    ...
    ) -> StorageResult<Vec<jansu_sans_io::delete_groups_response::DeletableGroupResult>> {
    ...
    ) -> StorageResult<Vec<jansu_sans_io::txn_offset_commit_response::TxnOffsetCommitResponseTopic>>
    {
...
async fn schema_failure_maps_and_keeps_offsets() -> Result<(), TestError> {
    ...
    let (inner, storage) = exactness_storage(
        ...
        Some(StorageError::Schema(Arc::new(
            jansu_schema::Error::SchemaValidation,
        ))),
    )
    .await?;
    let ctx = Context::with_state(storage.clone());

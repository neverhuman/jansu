...
async fn main() -> Result<ErrorCode> {
    ...
    Cli::main()
        ...
        .inspect_err(|err| match err {
            jansu_cli::Error::Cat(error) => match &**error {
                jansu_cat::Error::Client(_) => error!("{}", CLIENT_ERROR_MESSAGE),
                _ => error!("Unknown error occurred during command: {}", error),
            },
            jansu_cli::Error::Generate(error) => match error {
                jansu_generator::Error::Client(_) => error!("{}", CLIENT_ERROR_MESSAGE),
                _ => error!("Unknown error occurred during command: {}", error),
            },
            jansu_cli::Error::Perf(error) => match error {
                jansu_perf::Error::Client(_) => error!("{}", CLIENT_ERROR_MESSAGE),
                _ => error!("Unknown error occurred during command: {}", error),
            },
            jansu_cli::Error::Proxy(error) => match error {
                jansu_proxy::Error::Client(_) => error!("{}", CLIENT_ERROR_MESSAGE),
                _ => error!("Unknown error occurred during command: {}", error),
            },
            jansu_cli::Error::Topic(error) => match error {
                jansu_topic::Error::Client(_) => error!("{}", CLIENT_ERROR_MESSAGE),
                _ => error!("Unknown error occurred during command: {}", error),
            },

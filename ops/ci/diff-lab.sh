...
CARGO_TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/jansu-verify-phase04-contract}" \
    cargo test -p jansu-broker --test compatibility_contract --all-features -- --nocapture

JANSU_DIFFERENTIAL=1 \
JANSU_DIFF_ARTIFACT_DIR="${JANSU_DIFF_ARTIFACT_DIR:-target/differential}" \
CARGO_TARGET_DIR="${CARGO_TARGET_DIR_DIFF:-/tmp/jansu-verify-phase04-differential}" \

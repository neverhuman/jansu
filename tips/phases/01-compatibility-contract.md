...
- `jansu-broker/src/service/storage.rs`, `jansu-broker/src/service/coordinator.rs`, and `jansu-broker/src/service/auth.rs` define the broker's routed API surface.
- `jansu-storage/src/lib.rs` defines the `Storage` trait and storage-engine abstraction.
- Existing research notes in `tips/jansu_phases/` and `tips/stage_11/` are inputs, not official phase plans.
...
- Do not delete the research notes under `tips/jansu_phases/` or `tips/stage_11/`.
